...
export async function GET() {
  try {
    ...
    const row = await (db as any).siteSettings.findUnique({ where: { key: 'site_favicon_data' } })
    if (row && row.value) {
      ...
      if (parts.length === 2) {
        ...
        const contentType = match ? match[1] : 'image/x-icon'
    ...
    console.error('GET favicon error:', e)
...
export async function POST(request: Request) {
  try {
    ...
    const mimeType = file.type || 'image/x-icon'
    const dataUrl = `data:${mimeType};base64,${base64}`
    
    await (db as any).siteSettings.upsert({
      where: { key: 'site_favicon_data' },
      update: { value: dataUrl },
      create: { key: 'site_favicon_data', value: dataUrl }
    })

    const url = `/api/favicon?ts=${Date.now()}`
